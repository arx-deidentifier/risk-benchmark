...
/**
 * Implementation of the Normalized Non-Uniform Entropy measure with a lower bound. Inspired by:<br>
 * <br>
 * A. Gionis, T. Tassa, k-Anonymization with minimal loss of information, Trans Knowl Data Engineering 21 (2) (2009) 206–219.
 * 
 * @author Fabian Prasser
 */
public class UtilityMeasureNonUniformEntropyWithLowerBoundNormalized<T> extends UtilityMeasureAggregatable<T> {
    ...
    private String[][]                                           fullAnonOutputArray;
    /** Top transformation */
    private int[]                                                top;
    /** Header */
    ...
    public UtilityMeasureNonUniformEntropyWithLowerBoundNormalized(String[] header, String[][] input, Map<String, String[][]> hierarchies) {
        this(header, input, null, null, hierarchies, (AggregateFunction<T>) AggregateFunction.SUM);
    }
     ...
     * @param header
     * @param input
     * @param fullAnonOutputArray TODO
     * @param top TODO
     */
    public UtilityMeasureNonUniformEntropyWithLowerBoundNormalized(String[] header, String[][] input, String[][] fullAnonOutputArray, int[] top, Map<String, String[][]> hierarchies, AggregateFunction<T> function) {
        super(function);
        this.frequencyInput = getFrequency(header, input);
        this.input = input;
        this.fullAnonOutputArray = fullAnonOutputArray;
        this.top = top;
        this.header = header;
    ...
    protected double[] evaluateAggregatable(String[][] input, int[] transformation) {
        double[] res = eval(input, transformation);
        double[] max = eval(this.fullAnonOutputArray, top);
        double[] normalized = new double[header.length];
        for (int i = 0; i < header.length; i++) {
            normalized[i] = res[i] / max[i];
        }
        return normalized;
    }
     ...
     * @return
     */
    private double[] eval(String[][] output, int[] transformation) {
